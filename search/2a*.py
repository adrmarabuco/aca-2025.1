    ...
    def a_star(self, origem, destino):
        ...
        while self.estacao != self.destino:        
            ...
            if len(self.memoria) > 0 and self.rodada[0]["fim"] == self.memoria[-1][0]["inicio"]:
                ...
                self.memoria[-1][0]["custo_historico"] += (self.rodada[0]["custo_trecho"])
                # logging.info(f'agora: {self.memoria[-1][0]["custo_historico"]}')
                # logging.info(f'antes: {self.memoria[-1][0]["custo_total"]}')
                self.memoria[-1][0]["custo_total"] += (self.rodada[0]["custo_trecho"])
        ...
        for i, r in enumerate(self.memoria):
            if r[0]['volta']:
                trajeto.append([self.memoria[i-1][0]['inicio'],self.memoria[i-1][0]['linha_fim'], self.memoria[i-1][0]['custo_historico']])
                trajeto[-2][2] -= self.memoria[i-1][0]['custo_trecho']
